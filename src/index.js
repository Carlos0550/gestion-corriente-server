...
app.post("/make-deliver", async (req, res) => {
  ...
  const advanceOneMonth = dayjs(fecha_entrega)
    .add(1, "month")
...
app.get("/get-history", async (req, res) => {
  ...
  if (clientID) {
    try {
      ...
      return res.status(500).json({message: "Error interno del servidor: No se pudo traer el historial del cliente", error})
    }
  }else{
...
app.get("/get-view-vencimientos", async(req,res)=>{
  ...
  try {
    ...
    if (response.rows) {
      ...
      return res.status(400).json({message: "No hay vencimientos para mostrar"})
    }
  } catch (error) {
    return res.status(500).json({message: "Error del servidor: No se pudieron traer los vencimientos, recargue la página para intentarlo nuevamente"})
  }
})
...
app.post("/create-clients", async (req, res) => {
  ...
  const query1 = 'SELECT COUNT(*) AS existe FROM users WHERE LOWER(nombre_completo) = $1';
  const query2 = 'SELECT COUNT(*) AS existe FROM users WHERE dni = $1';
  const query3 = 'INSERT INTO users(nombre_completo, dni, telefono, apodo) VALUES($1, $2, $3, $4)';
  
  try {
    const nombreExiste = await clientSupabase.query(query1, [nombre_completo.toLowerCase()]);
    if (nombreExiste.rows[0].existe > 0) {
      return res.status(400).json({ message: "El nombre completo ya existe, por favor intente con otro." });
    }

    const dniExiste = await clientSupabase.query(query2, [dni]);
    if (dniExiste.rows[0].existe > 0) {
      return res.status(400).json({ message: "El DNI ya está registrado, por favor intente con otro." });
    }
    ...
    if (response.rowCount > 0) {
      ...
      return res.status(400).json({ message: "No se pudo crear el cliente, por favor intente nuevamente." });
    }
  } catch (error) {
    console.error("Error en la creación del cliente:", error);
    return res.status(500).json({ message: "Error interno del servidor: No se pudo crear el cliente, por favor intente nuevamente." });
  }
});
...
app.delete("/delete-deliver", async (req, res) => {
  ...
  try {
    ...
    if (response.rowCount === 1) {
      ...
      await clientSupabase.query("ROLLBACK");
      return res.status(404).json({ message: "Entrega no encontrado o ya eliminado" });
    }
  } catch (error) {
    await clientSupabase.query("ROLLBACK");
